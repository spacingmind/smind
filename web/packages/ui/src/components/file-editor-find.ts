...
/**
 * File Find's model (AC2): CodeMirror's own `@codemirror/search` owns
 * query, matching, selection, and replacement -- this only drives its
 * commands and republishes its state as a plain snapshot the shared
 * `FindBar` renders. Ported near-verbatim from Paseo's
 * `file-pane/find/model.web.ts`, which is itself pure `@codemirror/search`
 * usage with no React Native dependency -- the same CodeMirror, the same
 * package, the same API.
 *
 * The widget renders *outside* CodeMirror (`file-editor-find-bar.tsx`,
 * floating over the editor like the chat/terminal Find bars), so this
 * model's `extension` registers an inert placeholder panel purely to learn
 * when the built-in search state opens/closes/changes -- `basicSetup`'s own
 * default panel is never shown; ours replaces it.
 */
...
export class FileFindModel {
  ...
  readonly extension: Extension = [
    ...
    // itself, never the shared panel container -- CodeMirror's "Go to line" dialog
    // lives there too.
    EditorView.theme({ ".cm-panel.smind-file-find": { display: "none" } }),
    ...
    // basicSetup already binds the rest of searchKeymap (F3/Alt-G/...);
    // Mod-f is excluded here because smind's `pane.find` keyboard action
    // (keyboard/shortcuts.ts) claims it globally and calls `open` below --
    // leaving CodeMirror's own copy in would fire this model's open a
    // second, redundant time whenever that global handler doesn't
  ...
  readonly setReplacement = (replacement: string) => this.setQuery(this.snapshot.query, replacement);
  ...
  private setQuery(query: string, replacement: string) {
    this.view?.dispatch({
      effects: setSearchQuery.of(new SearchQuery({ search: query, replace: replacement, literal: true })),
    });
  }
  ...
  private createPanel(view: EditorView): Panel {
    ...
    return {
      ...
      update: (update: ViewUpdate) => {
        ...
        if (update.docChanged || update.selectionSet || queryChanged || update.geometryChanged || update.viewportChanged)
  ...
  private clearance(view: EditorView): number {
    ...
    const clearance = this.snapshot.placement === "top" ? box.bottom - editor.top : editor.bottom - box.top;
  ...
  private update(view: EditorView, recount: boolean) {
    ...
    const selection = view.state.selection.main;
    const current = this.matches.findIndex((match) => match.from === selection.from && match.to === selection.to) + 1;
    this.snapshot = {
      ...this.snapshot,
      query: query.search,
