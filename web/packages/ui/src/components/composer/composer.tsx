...
function DiffStatPill({ stat, onOpenDiff }: { stat: DiffStat; onOpenDiff: () => void }) {
  return (
    <button
      ...
      onClick={onOpenDiff}
      className="flex shrink-0 items-center gap-1.5 rounded-full border border-input bg-surface px-2.5 py-0.5 text-ui-xs text-foreground-muted transition-colors hover:bg-accent hover:text-foreground"
    >
        ...
        <ul data-testid="composer-queue" className="flex flex-col gap-1">
          {queued.map((text, index) => (
            <li
              ...
              className="flex items-center gap-2 rounded-md bg-surface px-2 py-1 text-ui-xs text-foreground-muted"
            >
