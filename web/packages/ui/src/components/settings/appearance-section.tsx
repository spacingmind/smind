    ...
    <div role="group" aria-label={groupLabel} className="inline-flex rounded-lg border border-input p-0.5">
      {options.map((opt) => (
        <Button
          ...
          className={cn("h-7 rounded-md px-3 text-ui-xs", opt.value === value && "bg-accent text-accent-foreground")}
        >
          {opt.label}
        </Button>
      ))}
  ...
  return (
    <div className="flex flex-col gap-6" data-testid="settings-section-appearance">
      <section className="flex flex-col gap-2">
        <h3 className="text-ui-sm font-medium text-foreground">Theme</h3>
        <StepGroup groupLabel="theme" options={THEME_OPTIONS} value={preference} onChange={setPreference} />
      </section>

      <section className="flex flex-col gap-4">
        <h3 className="text-ui-sm font-medium text-foreground">Font size</h3>
