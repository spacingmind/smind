    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        ...
        <div className="flex items-center gap-2">
          ...
          <p className="min-w-0 flex-1 truncate text-ui-sm font-medium" title={result?.path ?? ""}>
            {result?.path ?? "…"}
          </p>
        </div>
        ...
        {recentPaths.length > 0 && (
          <div className="flex flex-wrap items-center gap-1.5">
            <span className="text-ui-xs text-muted-foreground">Recent:</span>
            {recentPaths.map((path) => (
              <Button
                ...
                data-path={path}
                className="h-6 max-w-48 truncate px-2 text-ui-xs"
                title={path}
        ...
        {error && (
          <p className="flex items-center gap-1.5 text-ui-sm text-destructive">
        ...
        <ScrollArea className="h-64 rounded-md border">
          {loading && !result ? (
            <p className="flex items-center gap-1.5 p-3 text-ui-sm text-muted-foreground">
              <Loader2 className="size-3.5 animate-spin" />
              Loading…
            </p>
          ) : result && result.entries.length === 0 ? (
            <p className="p-3 text-ui-sm text-muted-foreground">No subdirectories.</p>
          ) : (
            <ul>
              {result?.entries.map((entry) => (
                <li key={entry.path}>
                  <button
                    ...
                    className="flex w-full items-center gap-1.5 px-3 py-1.5 text-left text-ui-sm hover:bg-accent"
                  >
                    {entry.isGitRepo ? (
