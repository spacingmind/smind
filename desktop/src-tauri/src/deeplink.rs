...
/// handle processes one `smind://` URL.
pub fn handle(app: &AppHandle, cache: WorkspaceCache, cfg: Config, raw: &str) {
    ...
    match link {
        DeepLink::WorkspaceTask { workspace_id, task_id } => {
            ...
            navigate(app, &cache, &cfg, task_id);
        }
...
fn navigate(app: &AppHandle, cache: &WorkspaceCache, cfg: &Config, task_id: i64) {
