...
func TestServer_RunLogs_PreExistingEventsStillDecode(t *testing.T) {
	...
	if len(logs.Events) != 3 {
		t.Fatalf("run.logs events = %+v, want 3", logs.Events)
	}
	if logs.Events[0].Type != "chunk" || logs.Events[0].Text != "hello " {
		t.Fatalf("event[0] = %+v, want chunk %q", logs.Events[0], "hello ")
	}
	if logs.Events[1].Type != "chunk" || logs.Events[1].Text != "world!" {
		t.Fatalf("event[1] = %+v, want chunk %q", logs.Events[1], "world!")
	}
	if logs.Events[2].Type != "done" || logs.Events[2].StopReason != "end_turn" {
		t.Fatalf("event[2] = %+v, want done/end_turn", logs.Events[2])
	}
...
func TestServer_RunRespondPermission_CrossConnection(t *testing.T) {
	...
	if resolved.Reason != "human" {
		t.Fatalf("permission_resolved Reason = %q, want %q -- a real run.respondPermission call must be distinguishable from an auto-resolution", resolved.Reason, "human")
	}
...
func TestServer_RunLogs_ShowsPermissionRequestAndResolved(t *testing.T) {
	...
	for _, e := range logs2.Events {
		if e.Type == "permission_resolved" {
			...
			if e.Reason != "human" {
				t.Fatalf("run.logs permission_resolved Reason = %q, want %q", e.Reason, "human")
			}
			if e.Text != "" {
