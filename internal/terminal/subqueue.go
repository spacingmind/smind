...
// one Subscribe call's live feed. This is internal/runs.subQueue's design
// copied verbatim (adapted to terminal.Event) rather than reused directly
// across packages -- it's an unexported type tied to its own package's
// Event, and the concurrency logic it encodes is exactly the kind of
// thing this project has already gotten subtly wrong three times this
// session (per the run-registry postmortem): a channel/queue close racing
// a concurrent send. Don't touch this without re-reading
// internal/runs/subqueue.go's doc comments first.
//
// It exists so record's broadcast to many subscribers never blocks on a
// slow one: push only ever appends to a slice and returns, regardless of
// whether pop is being called.
//
